...
class SincConv(nn.Module):
    ...
    def forward(self,x):
        ...
        # print("band_pass_filter", band_pass_filter.shape)
...
class HybridModel(nn.Module):
    def __init__(self,num_languages):
        ...
        # LSTM block
        hidden_size = 64
        self.lstm = nn.LSTM(input_size=465,hidden_size=hidden_size,bidirectional=False, batch_first=True) 
        self.dropout_lstm = nn.Dropout(p=0.3)
        # Linear softmax layer
        self.out_linear = nn.Linear(hidden_size,num_languages)
    def forward(self,x):
        x = x.view(-1, 1, 128, 261)
...
class Feature_Extractor(nn.Module):
    def __init__(self, d_args, device):
        ...
        self.Sinc_conv=SincConv(device=self.device,
			out_channels = d_args['filts'][0], # 20
			kernel_size = d_args['first_conv'], # 1024
...
class RawNet(nn.Module):
    def __init__(self, d_args, device, num_languages):
        super(RawNet, self).__init__()

        self.device=device
        self.feature_extractor = Feature_Extractor(d_args, device)
        self.classifier = Classifier(d_args)
        self.discriminator = HybridModel(num_languages).to(device)
    ...
    def forward(self, x, alpha):
        ...
        domain_output_logits, domain_output_softmax = self.discriminator(reverse_feature)

        return class_output, domain_output_logits, domain_output_softmax
    ...
    def get_parameters_wioD(self, base_lr=1.0) -> List[Dict]:
        """A parameter list which decides optimization hyper-parameters,
            such as the relative learning rate of each layer
        """
        params = [  
            {"params": self.feature_extractor.parameters(), "lr": 1.0 * base_lr},
